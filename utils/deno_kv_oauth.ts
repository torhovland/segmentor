...
function setAccessTokenCookie(headers: Headers, accessToken: string) {
  setCookie(headers, {
...
export function getAccessTokenCookie(headers: Headers) {
  return getCookies(headers)[ACCESS_TOKEN_COOKIE_NAME];
}
...
export function setCallbackHeaders(
  headers: Headers,
  sessionId: string,
  accessToken: string,
) {
